 ...
 */
    ...
    if ($this->params->get('buf_leading_bs_sm', 3)) {
        $bsclass .= " row-cols-sm-" . $this->params->get('buf_leading_bs_sm', 3);
    }

    if ($this->params->get('buf_leading_bs_md', 3)) {
        $bsclass .= " row-cols-md-" . $this->params->get('buf_leading_bs_md', 3);
    }

    if ($this->params->get('buf_leading_bs_lg', 3)) {
        $bsclass .= " row-cols-lg-" . $this->params->get('buf_leading_bs_lg', 3);
    }

    if ($this->params->get('buf_leading_bs_xl', 3)) {
        $bsclass .= " row-cols-xl-" . $this->params->get('buf_leading_bs_xl', 3);
    }

    if ($this->params->get('buf_leading_bs_gutter', 4)) {
        $bsclass .= " g-" . $this->params->get('buf_leading_bs_gutter', 4);
    }
    ...
    <?php if (!empty($this->lead_items)) : ?>
        <?php foreach ($this->lead_items as &$item) : ?>
            <div class="col <?php //echo $bsclass;?>">
                <div class="card item-leading <?php echo $card_class; ?><?php echo $item->state == 0 ? ' system-unpublished' : null; ?><?php echo $item->featured == 1 ? ' card-featured' : null; ?>" itemprop="blogPost" itemscope itemtype="https://schema.org/BlogPosting">
                    <?php
                    $item->isleading = true;
                    $this->item = &$item;
...
<?php 
    $show_items = true;
    if ($this->params->get('bufbs_subcategory_as_item', 1) == 1 && count($this->children[$this->category->id]) > 0){
        $show_items = false;
    }

    if (!empty($this->intro_items) && $show_items) : ?>
    <?php foreach ($this->intro_items as $key => &$item) : ?>
        <div class="col <?php //echo $bsclass;?>">
            <div class="card item <?php echo $card_class; ?><?php echo $item->state == 0 ? ' system-unpublished' : null; ?><?php echo $item->featured == 1 ? ' card-featured' : null; ?>" itemprop="blogPost" itemscope itemtype="https://schema.org/BlogPosting">
                <?php
                $item->isleading = false;
                $this->item = &$item;
                echo $this->loadTemplate('item');
                ?>
            </div><!-- end card -->
...
<?php if ($this->maxLevel != 0 && !empty($this->children[$this->category->id])) : ?>
    <div class="com-content-category-blog__children cat-children">
        ...
        <?php if ($this->params->get('bufbs_subcategory_as_item', 1) == 1) : ?>
            <?php echo $this->loadTemplate('subasitem'); ?>
        <?php else : ?>
            <?php echo $this->loadTemplate('subasitem'); ?>
        <?php endif; ?>
