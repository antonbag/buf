...
class BufFavicon
{
    ...
    public static function _generateAndroidJson($template_name, $fav_path, $iconsFolderUrl)
    {
        ...
        $icos = array('name' => $template_name, 'icons' => array(
            array(
    ...
    public static function addFaviconLinks(): void
    {
        ...
        // Add additional SVG favicon with image helper if needed
        $buf_favicon_svg = $templateparams->get('buf_favicon_svg', '');
        if ($buf_favicon_svg !== '') {
            $svg_icon = HTMLHelper::cleanImageURL($buf_favicon_svg);
            if ($svg_icon && isset($svg_icon->url)) {
            $svg_url = Uri::root() . $svg_icon->url;
            $ms_color = $templateparams->get('buf_mscolor', '#57616d');
            
            $doc->addHeadLink(
                HTMLHelper::_('image', $svg_url, '', [], true, 1),
                'icon',
                'rel',
                ['type' => 'image/svg+xml']
            );
